...
For us to issue the token, we need to issue a change trust transaction between the **distribution** account and the **issuance** account. `You can do that through this handy web interface <http://laboratory.tee-coin.com/#txbuilder?network=test>`_.
